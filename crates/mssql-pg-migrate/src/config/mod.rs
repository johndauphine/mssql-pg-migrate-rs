...
impl SourceConfig {
    ...
    /// Build the OPTION clause for SQL Server query hints.
    /// Returns an empty string if no hints are configured.
    ///
    /// # Safety
    /// This function is safe from SQL injection because:
    /// - `query_hint_maxdop` is `Option<u8>` - a numeric type, clamped to 0-64
    /// - `query_hint_fast` is `Option<usize>` - a numeric type
    /// Both are formatted directly as numbers with no string interpolation.
    pub fn build_query_hints(&self) -> String {
        let mut hints = Vec::new();

        if let Some(maxdop) = self.query_hint_maxdop {
            // Clamp MAXDOP to valid SQL Server range (0-64)
            // 0 means use server default, 1 disables parallelism
            let maxdop = maxdop.min(64);
            hints.push(format!("MAXDOP {}", maxdop));
        }

        if let Some(fast) = self.query_hint_fast {
            // FAST N optimizes for returning first N rows quickly
            // Value is usize, always a valid non-negative integer
            hints.push(format!("FAST {}", fast));
        }
...
"#;
  ...
  "migration": {
    ...
    #[test]
    fn test_build_query_hints_empty() {
        let config = Config::from_yaml(VALID_YAML).unwrap();
        assert_eq!(config.source.build_query_hints(), "");
    }

    #[test]
    fn test_build_query_hints_maxdop_only() {
        let yaml = r#"
source:
  type: mssql
  host: localhost
...
"#;
        ...
        assert_eq!(config.source.build_query_hints(), " OPTION (MAXDOP 4)");
    }

    #[test]
    fn test_build_query_hints_fast_only() {
        let yaml = r#"
source:
  type: mssql
  host: localhost
...
"#;
        let config = Config::from_yaml(yaml).unwrap();
        assert_eq!(config.source.build_query_hints(), " OPTION (FAST 10000)");
    }

    #[test]
    fn test_build_query_hints_both() {
        let yaml = r#"
source:
  type: mssql
  host: localhost
...
"#;
        let config = Config::from_yaml(yaml).unwrap();
        assert_eq!(config.source.build_query_hints(), " OPTION (MAXDOP 8, FAST 50000)");
    }

    #[test]
    fn test_build_query_hints_maxdop_clamped() {
        let yaml = r#"
source:
  type: mssql
  host: localhost
...
"#;
        let config = Config::from_yaml(yaml).unwrap();
        // MAXDOP 255 should be clamped to 64
        assert_eq!(config.source.build_query_hints(), " OPTION (MAXDOP 64)");
    }
}
