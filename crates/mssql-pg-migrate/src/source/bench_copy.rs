...
mod bench {
    ...
    async fn bench_select_reads() {
        ...
        let pool = PgSourcePool::new(&config, 8)
            .await
            .expect("Failed to connect");

        // Get table schema
        let tables: Vec<Table> = pool
            .extract_schema("public")
            .await
            .expect("Failed to extract schema");
        let votes_table = tables
            .iter()
            .find(|t| t.name.to_lowercase() == "votes")
            .expect("Votes table not found");

        println!("\n=== SELECT Benchmark ===");
        println!(
            "Table: public.{} ({} rows)",
            votes_table.name, votes_table.row_count
        );

        let columns: Vec<String> = votes_table.columns.iter().map(|c| c.name.clone()).collect();
        let col_types: Vec<String> = votes_table
            .columns
            .iter()
        ...
        let col_list = columns
            ...
            .join(", ");
        let sql = format!(
            "SELECT {} FROM public.\"{}\" ORDER BY \"Id\" LIMIT 1000000",
            col_list, votes_table.name
        );
        ...
        for i in 0..iterations {
            ...
            let rows = pool
                .query_rows_fast(&sql, &columns, &col_types)
                .await
                .expect("Query failed");
    ...
    async fn bench_compare_copy_vs_select() {
        ...
        for &size in &test_sizes {
            ...
            let col_list = columns
                ...
                .join(", ");
            let sql = format!(
                "SELECT {} FROM public.\"{}\" WHERE \"Id\" >= 1 AND \"Id\" <= {} ORDER BY \"Id\"",
                col_list, votes_table.name, size
            );

            let start = Instant::now();
            let select_rows = pool
                .query_rows_fast(&sql, &columns, &col_types)
                .await
            ...
            println!(
                "SELECT: {} rows in {:?} ({:.0} rows/s)",
                select_rows.len(),
                select_time,
                select_rate
            );
            println!(
                "COPY:   {} rows in {:?} ({:.0} rows/s)",
                copy_rows, copy_time, copy_rate
            );
